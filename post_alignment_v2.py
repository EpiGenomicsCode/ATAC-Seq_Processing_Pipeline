...
chrM_file = "/home/ubuntu/atac_pip/chrM.bed"
picard = "/home/ubuntu/atac_pip/picard.jar"
...
def alignment_adjustment(input_bam:str, sample_name: str, output_dir:str) -> None:
    ...
    cmd_add_rg_group = f'java -jar {picard} AddOrReplaceReadGroups -I {new_bam} -O {output_dir}/{sample_name}_noMT_rg.bam -LB lib -PL Element -PU unit -SM {sample_name}'
    subprocess.run(cmd_add_rg_group, shell=True, check=True)
    new_bam=f"{output_dir}/{sample_name}_noMT_rg.bam"
    cmd_dups_removal = f'java -jar {picard} MarkDuplicates -I {new_bam} -O {output_dir}/{sample_name}_noMT_noDups.bam -M {output_dir}/{sample_name}_dups.txt -REMOVE_DUPLICATES true'
