...
for BAM_FILE in "$post_align_output_dir"/*.bam; do
  # Extract the sample name from the BAM file (removes path and extension)
  SAMPLE_NAME=$(basename "$BAM_FILE" .bam)
  
  # Create a directory for the current sample's MACS2 output
  SAMPLE_OUTPUT_DIR="$macs2_output_dir/$SAMPLE_NAME"
  mkdir -p "$SAMPLE_OUTPUT_DIR"

  echo "Processing sample: $SAMPLE_NAME"

  # Run MACS2 for paired-end reads (-f BAMPE option)
  macs2 callpeak \
    -t "$BAM_FILE" \
    -f BAMPE \
    -g hs \
  ...
  echo "MACS2 peak calling complete for $SAMPLE_NAME"
done
